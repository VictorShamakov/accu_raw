...
```{r}
df.spd <- na.omit(df.spd)
df.spd <- df.spd[df.spd$gender == "Женский",]

df.spd$science_type <- factor(df.spd$science_type, labels = c("Natural", "Social", "Tech"))
df.spd <- cbind(df.spd, model.matrix(~ science_type - 1, data = df.spd)) # one hot encoding

df.spd$direction <- factor(df.spd$direction)
...
```{r}
save(df, file = "./data/envs/4_cluster_analysis/df.spd.female.RData")
```
