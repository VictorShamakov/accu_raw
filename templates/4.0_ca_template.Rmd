...
```{r}
df <- df.{{stage}}
df <- select(df, -session_id, -test_id, -ratio, -clicked_through_targets, -combo_clicked_through_targets, -stage)
```

{% elif stage == 'acc_spd' %}

```{r}
pd <- select(df.acc, user_id, gender)
...
df.acc_spd <- merge(df.acc_spd, pd, by = "user_id")
df.acc_spd <- select(df.acc_spd, user_id, gender, everything())
```
