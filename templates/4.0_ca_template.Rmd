...
{% if stage == "acc" or stage == "spd" %}

```{r}
df <- select(df.{{stage}}, -session_id, -test_id, -ratio, -clicked_through_targets, -combo_clicked_through_targets, -stage)
```

{% elif stage == 'acc_spd' %}

```{r}
info <- select(df.acc, user_id, session_id, test_id, ratio, gender)

df.acc <- select(df.acc, user_id, everything(), -session_id, -test_id, -ratio, -gender, -clicked_through_targets, -combo_clicked_through_targets, -stage)
names(df.acc)[-1] <- paste0(names(df.acc)[-1], ".acc")

df.spd <- select(df.spd, user_id, everything(), -session_id, -test_id, -ratio, -gender, -clicked_through_targets, -combo_clicked_through_targets, -stage)
names(df.spd)[-1] <- paste0(names(df.spd)[-1], ".spd")

df.acc_spd <- merge(df.acc, df.spd, by = "user_id")

df.acc_spd <- merge(info, df.acc_spd, by = "user_id")
```

```{r}
df <- select(df.acc_spd, -session_id, -test_id, -ratio)
```
...
df.acc <- select(df.acc, everything(), -user_id, -session_id, -test_id, -ratio, -gender, -clicked_through_targets, -combo_clicked_through_targets, -stage)

df.spd <- select(df.spd, everything(), -user_id, -session_id, -test_id, -ratio, -gender, -clicked_through_targets, -combo_clicked_through_targets, -stage)

df.diff <- df.spd - df.acc
df.diff <- cbind(info, df.diff)
```

```{r}
df <- select(df.diff, -session_id, -test_id, -ratio)
```
...
### Кликеры
```{r}
cluster_clickers.{{stage}} <- as.data.frame(matrix("clickers", nrow = nrow(clickers.{{stage}}), ncol = length(names(df)[grep("clust", names(df))])))
colnames(cluster_clickers.{{stage}}) <- names(df)[grep("clust", names(df))]

clickers.{{stage}} <- cbind(clickers.{{stage}}, cluster_clickers.{{stage}})

clickers.{{stage}} <- na.omit(clickers.{{stage}})
clickers.{{stage}} <- cbind(clickers.{{stage}}, model.matrix(~ gender - 1, data = clickers.{{stage}}))

df <- rbind(df, select(clickers.{{stage}}, names(df)))
```
...
df <- merge(df.{{stage}}, select(df, user_id, contains("clust")), by = "user_id")
```

{% if gender == 'male' %}
```{r}
save(df, file = "./data/envs/4_cluster_analysis/df.{{stage}}.male.RData")
```
{% elif gender == "female" %}
```{r}
save(df, file = "./data/envs/4_cluster_analysis/df.{{stage}}.female.RData")
```
{% else %}
```{r}
save(df, file = "./data/envs/4_cluster_analysis/df.{{stage}}.RData")
```
{% endif %}
