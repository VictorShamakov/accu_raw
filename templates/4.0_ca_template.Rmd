...
{% if gender == 'male' %}
```{r}
df.{{stage}} <- df.{{stage}}[df.{{stage}}$gender == "M",]
```
{% elif gender == "female" %}
```{r}
df.{{stage}} <- df.{{stage}}[df.{{stage}}$gender == "F",]
```
{% else %}
```{r}
df.{{stage}}$gender_num <- as.numeric(df.{{stage}}$gender) - 1
```
{% endif %}
...
df.{{stage}}[, logarithmized_vars] <- lapply(df.{{stage}}[, logarithmized_vars], function(x) {
