...
#### Normal test
```{r}
st <- shapiro.test(df.acc${{feature.short_name}});print(st)
check_normal_distribution(st)
ks <- ks.test(df.acc${{feature.short_name}}, "pnorm");print(ks)
check_normal_distribution(ks)

st <- shapiro.test(df.spd${{feature.short_name}});print(st)
check_normal_distribution(st)
ks <- ks.test(df.spd${{feature.short_name}}, "pnorm");print(ks)
check_normal_distribution(ks)
```

```{r}
diff_data <- df.acc${{feature.short_name}} - df.spd${{feature.short_name}}
...
friedman_test(df.test, {{feature.short_name}} ~ {{group}} | user_id)
friedman_effsize(df.test, {{feature.short_name}} ~ {{group}} | user_id)

t_test_results <- t.test(df.acc${{feature.short_name}}, df.spd${{feature.short_name}}, paired = TRUE);t_test_results
wilcox_test_results <- wilcox.test(df.acc${{feature.short_name}}, df.spd${{feature.short_name}}, paired = TRUE);wilcox_test_results
```
