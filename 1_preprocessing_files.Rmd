...
ideal_dist <- sum(get_distance(ideal_coords$x_mouse_pos, ideal_coords$y_mouse_pos))

ggplot(data = ideal_coords, aes(x_mouse_pos, y_mouse_pos)) + 
    scale_y_reverse() + xlab("x") + ylab("y") + theme_bw() + 
    geom_path(alpha = I(0.9), linewidth=0.5, color = "deepskyblue") +
    geom_point(aes(x_mouse_pos, y_mouse_pos), color="black", fill="white", shape=21, size=11, alpha=0.7) +
    ...
    geom_text(
          label=paste0("Время: ", "Расстояние: ", ideal_dist, " px\n"), 
          x=0,
          y=0,
          size=5,
...
ggplot(ast_long.wo_ctt, aes(velocity, miss_distance, group=stage)) + geom_point(aes(col=stage)) + stat_smooth(method = "lm") +
  xlab("Скорость, px/s") + ylab("Расстояние промахов, px") +
  ggtitle("Компромисс между скоростью и точностью на каждом этапе") 
