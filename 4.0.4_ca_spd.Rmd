---
title: "Кластерный анализ. Задание на скорость"
...
df.spd <- cbind(df.spd, model.matrix(~ gender - 1, data = df.spd))

df.spd$science_type <- factor(df.spd$science_type, labels = c("Natural", "Social", "Tech"))
df.spd <- cbind(df.spd, model.matrix(~ science_type - 1, data = df.spd))
